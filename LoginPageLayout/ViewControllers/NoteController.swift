...
class NoteController: UIViewController, UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        if !isNew{
            ...
            navigationItem.rightBarButtonItem = UIBarButtonItem(image: Constants.ImageConstant.remind, style: .plain, target: self, action: #selector(navigatePicker))
        }
    ...
    func configureUI(){
        ...
        let archiveButton = UIBarButtonItem(image: note?.isArchive ?? false ?  Constants.ImageConstant.unarchive : Constants.ImageConstant.archive, style: .plain, target: self, action: #selector(archiveNote))
        
        let remindButton = UIBarButtonItem(image: Constants.ImageConstant.remind, style: .plain, target: self, action: #selector(navigatePicker))
